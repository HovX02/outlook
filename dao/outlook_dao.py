...
def _migrate_schema_v3(conn: sqlite3.Connection) -> None:
    ...
    if not cols:
        return
    from service.resource.proxy.proxy_utils import infer_country_from_template
...
def migrate_legacy_files(accounts_dir: Path) -> dict[str, int]:
    ...
    try:
        ...
        from service.account.account_store import upsert_account_dict, upsert_meta_dict
